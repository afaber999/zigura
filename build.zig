...
pub fn build(b: *std.Build) void {
    ...
    // wasm.addIncludePath(b.path("src/lib/stb_image/"));
    // wasm.addCSourceFile(.{
    // .file = b.path("src/lib/stb_image/stb_image.c"),
    // //.flags = &.{"-freference-trace"},
    // //.c_source_flags = &[_][]const u8{ "-freference-trace" },
    // });
    // wasm.linkLibC();

    //copy HTML related files to output directory
    const install_html = b.addInstallDirectory(.{
        .source_dir = b.path("html"),
        .install_dir = .{ .custom = wasm_target_dir },
        .install_subdir = "",
    });
    ...
    const exe = b.addExecutable(.{
        ...
        .root_source_file = b.path("src/demos_native.zig"),
        .target = target,
        .optimize = optimize,
    });
